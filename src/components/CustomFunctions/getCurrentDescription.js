...
export default function getCurrentDescription(weeklyWeatherForecast){
    ...
    for(let i = 1; i < daysWeather.length; i++){
        ...
        if(hour1 >= currentHour && currentHour >= hour2 ){
            const description = hour1 - currentHour > currentHour - hour2 ? daysWeather[i].weather[0].description : daysWeather[i-1].weather[0].description
            const currentTemp = hour1 - currentHour > currentHour - hour2 ? daysWeather[i].main.temp : daysWeather[i-1].main.temp
